...
{
  ros::init(argc, argv, "test_custom_2_node");
...
bool success= static_cast<bool>(group.plan(my_plan));
  ...
  target_pose1.position.x = 1.26622;
  target_pose1.position.y = -0.577378;
  target_pose1.position.z = 0.79815;;  
  ocm.link_name = "link_06";  
  ...
  start_pose2.position.x = 2.55;
  start_pose2.position.y = -0.05;
  start_pose2.position.z = 1.7;
  ...
  // When don: Unable to solve the planning probleme with the path constraint be sure to clear it.
  ...
  group.setPoseTarget(target_pose3); // up and out

  target_pose3.position.y -= 0.2;
  waypoints.push_back(target_pose3);
  group.setPoseTarget(target_pose3);  // left

  target_pose3.position.z -= 0.2;
  target_pose3.position.y += 0.2;
  target_pose3.position.x -= 0.2;
  waypoints.push_back(target_pose3); 
  group.setPoseTarget(target_pose1); // down and right (back to start)
